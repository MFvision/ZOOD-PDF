...
fn mutate(rng: &mut Rng, data: &[u8]) -> Vec<u8> {
    ...
    match rng.below(5) {
        ...
        3 => {
            ...
            let big = ["99999999999", "-5", "0", "65535", "4294967295", "1e308", "NaN"][rng.below(7)];
...
fn mutate_part(rng: &mut Rng, pkg: &[u8]) -> Vec<u8> {
    ...
    let parts: Vec<(String, Vec<u8>)> = names
        ...
        .map(|(i, n)| {
            ...
            let data = if i == target { mutate(rng, &data) } else { data };
            (n.clone(), data)
        })
        .collect();
...
fn mutated_inputs_never_panic() {
    let cases: usize = std::env::var("WARRAQ_CREATE_SMOKE_CASES")
        ...
        .unwrap_or(60);
    let names = [
        "report.docx", "sales.xlsx", "deck.pptx", "guide.md", "page.html", "data.csv", "notes.txt",
        "photo.jpg", "logo.png", "scan.tif",
    ];
    ...
    for name in names {
        ...
        for k in 0..cases {
            let input = if ooxml && k % 2 == 0 { mutate_part(&mut rng, &data) } else { mutate(&mut rng, &data) };
            let t = Instant::now();
            let r = create(
                &[(FileSpec { name: name.into(), kind: None }, input)],
                &CreateOptions { merge: true, page_numbers: k % 3 == 0, ..CreateOptions::default() },
            );
            assert!(t.elapsed() < Duration::from_secs(20), "{name} case {k} took {:?}", t.elapsed());
            match r {
                Ok(v) => {
                    ok += 1;
...
fn hostile_constructions_are_refused_or_bounded() {
    ...
    let run = |name: &str, bytes: Vec<u8>| create(&[(FileSpec { name: name.into(), kind: None }, bytes)], &opts);
    ...
    let wide = format!("<table><tr>{}</tr><tr><td colspan=\"65535\" rowspan=\"65535\">x</td></tr></table>", "<td>c</td>".repeat(10_000));
    assert!(run("wide.html", wide.into_bytes()).is_ok());
    // Deep lists in Markdown.
    let deep: String = (0..500).map(|i| format!("{}- item\n", "  ".repeat(i))).collect();
